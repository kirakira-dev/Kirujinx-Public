...
namespace Ryujinx.HLE.HOS.Services.Sockets.Bsd.Impl
{
    class ManagedSocket : ISocket
    {
        ...
        public LinuxError Connect(IPEndPoint remoteEndPoint)
        {
            ...
            {
                // Handle non-blocking connect: check for WOULDBLOCK/INPROGRESS based on platform
                if (!Blocking && WinSockHelper.IsNonBlockingError(exception.ErrorCode))
                {
