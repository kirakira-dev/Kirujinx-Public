...
namespace Ryujinx.HLE.HOS.Services.Sockets.Bsd.Impl
{
    static class WinSockHelper
    {
        ...
        /// <summary>
        /// Checks if the given error code represents a non-blocking operation in progress.
        /// This handles platform-specific error codes for Windows, macOS, and Linux.
        /// </summary>
        public static bool IsNonBlockingError(int errorCode)
        {
            if (OperatingSystem.IsWindows())
            {
                // Windows: WSAEWOULDBLOCK (10035) or WSAEINPROGRESS (10036)
                return errorCode == (int)WsaError.WSAEWOULDBLOCK || errorCode == (int)WsaError.WSAEINPROGRESS;
            }
            else if (OperatingSystem.IsMacOS())
            {
                // macOS: EAGAIN (35), EINPROGRESS (36), EALREADY (37)
                return errorCode == 35 || errorCode == 36 || errorCode == 37;
            }
            else
            {
                // Linux: EAGAIN (11), EWOULDBLOCK (11), EINPROGRESS (115), EALREADY (114)
                return errorCode == 11 || errorCode == 115 || errorCode == 114;
            }
        }
