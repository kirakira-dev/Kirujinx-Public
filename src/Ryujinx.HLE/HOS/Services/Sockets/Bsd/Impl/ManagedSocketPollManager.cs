        ...
        public LinuxError Poll(List<PollEvent> events, int timeoutMilliseconds, out int updatedCount)
        {
            ...
            foreach (PollEvent evnt in events)
            {
                if (evnt.FileDescriptor is ManagedSocket ms)
                {
                    ...
                    if ((evnt.Data.InputEvents & PollEventTypeMask.Input) != 0 || evnt.Data.InputEvents == 0)
                    {
            ...
            {
                Logger.Info?.Print(LogClass.ServiceBsd, $"Received ObjectDisposedException, socket may have been closed.");
            }
