...
namespace Ryujinx.HLE.HOS.Services.Sockets.Bsd
{
    ...
    class IClient : IpcService
    {
        ...
        private ResultCode WriteBsdResult(ServiceCtx context, int result, LinuxError errorCode = LinuxError.SUCCESS, [CallerMemberName] string caller = null)
        {
            if (errorCode != LinuxError.SUCCESS)
            {
                if (errorCode != LinuxError.EWOULDBLOCK)
                {
                    Logger.Warning?.Print(LogClass.ServiceBsd, $"{caller}: Operation failed with error {errorCode}.");
                }
...
#pragma warning restore IDE0059
            ...
            for (int i = 0; i < fdsCount; i++)
            {
                ...
                {
                    events[i].Data.OutputEvents |= PollEventTypeMask.Invalid;
                    invalidCount++;

                    Logger.Info?.Print(LogClass.ServiceBsd, $"Invalid fd {pollEventData.SocketFd}, treating as invalid event.");
                }
            }
        ...
        public ResultCode SendTo(ServiceCtx context)
        {
            ...
            if (socket != null)
            {
                // If no destination address buffer is provided (bufferSize == 0 or bufferPosition == 0),
                // use Send instead of SendTo. This is valid for connected sockets.
                if (bufferSize == 0 || bufferPosition == 0)
                {
                    errno = socket.Send(out result, sendBuffer, socketFlags);
                }
                else
...
#pragma warning restore IDE0059
        ...
        {
            if (!context.RequestData.TryReadInt32(out int fd))
            {
                return WriteBsdResult(context, -1, LinuxError.EINVAL);
            }
            ...
            if (sendSize == 0)
            {
                return WriteBsdResult(context, 0, LinuxError.SUCCESS);
            }
...
#pragma warning restore IDE0059
            ...
            if (!context.RequestData.TryReadStruct(out timeout))
            {
                timeout = new TimeVal { TvSec = 0, TvUsec = 0 };
            }
