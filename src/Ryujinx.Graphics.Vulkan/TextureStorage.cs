...
namespace Ryujinx.Graphics.Vulkan
{
    class TextureStorage : IDisposable
    {
            ...
            Auto<MemoryAllocation> foreignAllocation = null)
        {
            ...
            int maxSize = Math.Max(info.Width, Math.Max(info.Height, (int)depth));
            int maxLevels = 1 + BitOperations.Log2((uint)maxSize);
            uint levels = (uint)Math.Min(info.Levels, maxLevels);
