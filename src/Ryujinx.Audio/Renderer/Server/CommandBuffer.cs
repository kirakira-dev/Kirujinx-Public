...
namespace Ryujinx.Audio.Renderer.Server
{
    ...
    public class CommandBuffer
    {
        ...
        private static bool _poolsPrewarmed = false;

        public static void PrewarmCommandPools()
        {
            if (_poolsPrewarmed)
            {
                return;
            ...
            _pcmInt16DataSourceCommandVersion1Pool.Prewarm(PrewarmCount);
            _pcmFloatDataSourceCommandVersion1Pool.Prewarm(PrewarmCount);
            _adpcmDataSourceCommandVersion1Pool.Prewarm(PrewarmCount);
            _dataSourceVersion2CommandPool.Prewarm(PrewarmCount);
            _volumeCommandPool.Prewarm(PrewarmCount * 2);
            _volumeRampCommandPool.Prewarm(PrewarmCount);
            _biquadFilterCommandPool.Prewarm(PrewarmCount * 2);
            _mixCommandPool.Prewarm(PrewarmCount * 4);
            _mixRampCommandPool.Prewarm(PrewarmCount);
            _mixRampGroupedCommandPool.Prewarm(PrewarmCount);
            _depopPrepareCommandPool.Prewarm(PrewarmCount);
            _depopForMixBuffersCommandPool.Prewarm(PrewarmCount);
            _delayCommandPool.Prewarm(PrewarmCount);
            _upsampleCommandPool.Prewarm(PrewarmCount);
            _downMixSurroundToStereoCommandPool.Prewarm(PrewarmCount);
            _auxiliaryBufferCommandPool.Prewarm(PrewarmCount);
            _deviceSinkCommandPool.Prewarm(PrewarmCount);
            _circularBufferSinkCommandPool.Prewarm(PrewarmCount);
            _reverbCommandPool.Prewarm(PrewarmCount);
            _reverb3dCommandPool.Prewarm(PrewarmCount);
            _performanceCommandPool.Prewarm(PrewarmCount);
            _clearMixBufferCommandPool.Prewarm(PrewarmCount * 2);
            _copyMixBufferCommandPool.Prewarm(PrewarmCount * 2);
            _limiterCommandVersion1Pool.Prewarm(PrewarmCount);
            _limiterCommandVersion2Pool.Prewarm(PrewarmCount);
            _multiTapBiquadFilterCommandPool.Prewarm(PrewarmCount);
            _captureBufferCommandPool.Prewarm(PrewarmCount);
            _compressorCommandPool.Prewarm(PrewarmCount);
            _biquadFilterAndMixCommandPool.Prewarm(PrewarmCount);
            _multiTapBiquadFilterAndMixCommandPool.Prewarm(PrewarmCount);
            _fillBufferCommandPool.Prewarm(PrewarmCount);

            _poolsPrewarmed = true;
        }
