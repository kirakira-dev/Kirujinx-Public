...
namespace Ryujinx.Ava.UI.ViewModels
{
    public partial class SettingsViewModel : BaseModel
    {
        ...
        public bool EnableFullPptc { get; set; }
        public bool EnableOnDemandUsage { get; set; }
        public bool PrioritizeSocketConnections { get; set; }
        public bool LockTo30Fps { get; set; }
        ...
        public bool IsCustomResolutionScaleActive => _resolutionScale == 6;
        public bool IsScalingFilterActive => _scalingFilter == (int)Ryujinx.Common.Configuration.ScalingFilter.Fsr;

        private static readonly float[] ResolutionScalePresets = { 0.5f, 0.75f, 1.0f, 2.0f, 3.0f, 4.0f };

        /// <summary>
        /// Converts a resolution scale index to the actual scale value.
        /// </summary>
        private static float GetResolutionScaleValue(int index)
        {
            if (index >= 0 && index < ResolutionScalePresets.Length)
            {
                return ResolutionScalePresets[index];
            }
            return 1.0f; // Default to native
        }

        /// <summary>
        /// Finds the index for a given resolution scale value.
        /// Returns 7 (custom) if not found in presets.
        /// </summary>
        private static int GetResolutionScaleIndex(float scaleValue)
        {
            for (int i = 0; i < ResolutionScalePresets.Length; i++)
            {
                if (Math.Abs(ResolutionScalePresets[i] - scaleValue) < 0.01f)
                {
                    return i;
                }
            }
        ...
        public float GameListBackgroundOpacity { get; set; }
        public bool EnableCustomAccentColor { get; set; }

        // Available accent color presets
        public string[] AccentColorPresets { get; } = new[]
        {
            "#FF00FABB", // Kirujinx Teal (default)
            "#FF2EEAC9", // Switch Green
            "#FFFF4554", // Nintendo Red
            "#FF6483F5", // Blue
            "#FFA855F7", // Purple
            "#FFFBBF24", // Yellow
            "#FFFF6B6B", // Coral
        ...
        public void LoadCurrentConfiguration(bool global = false)
        {
            ...
            // Theme customization (TODO: Add configuration support)
            // AccentColor = config.UI.AccentColor.Value;
            // GameListBackgroundOpacity = config.UI.GameListBackgroundOpacity.Value;
            // EnableCustomAccentColor = config.UI.EnableCustomAccentColor.Value;
            ...
            LockTo30Fps = config.Graphics.LockTo30Fps;
            EnablePerformanceMode = config.Graphics.EnablePerformanceMode;
            ...
            EnableFullPptc = config.System.EnableFullPtc;
            EnableOnDemandUsage = config.System.EnableOnDemandUsage;
            PrioritizeSocketConnections = config.System.PrioritizeSocketConnections;
            ...
            // Load resolution scale with new preset support
            if (config.Graphics.ResScale == -1)
            {
                // Custom scale - check if it matches a preset
                int presetIndex = GetResolutionScaleIndex(config.Graphics.ResScaleCustom);
                ResolutionScale = presetIndex;
                CustomResolutionScale = config.Graphics.ResScaleCustom;
            }
            else
            {
                // Legacy integer scale (1-4) - map to new indices (3-6)
                ResolutionScale = config.Graphics.ResScale + 2; // 1->3, 2->4, 3->5, 4->6
                CustomResolutionScale = config.Graphics.ResScaleCustom;
            }
        ...
        public void SaveSettings(bool global = false)
        {
            ...
            // Theme customization (TODO: Add configuration support)
            // config.UI.AccentColor.Value = AccentColor;
            // config.UI.GameListBackgroundOpacity.Value = GameListBackgroundOpacity;
            // config.UI.EnableCustomAccentColor.Value = EnableCustomAccentColor;
            ...
            config.System.EnableFullPtc.Value = EnableFullPptc;
            config.System.EnableOnDemandUsage.Value = EnableOnDemandUsage;
            config.System.PrioritizeSocketConnections.Value = PrioritizeSocketConnections;
            ...
            config.Graphics.LockTo30Fps.Value = LockTo30Fps;
            config.Graphics.EnablePerformanceMode.Value = EnablePerformanceMode;
            ...
            if (ResolutionScale == 6)
            {
                config.Graphics.ResScale.Value = -1;
                config.Graphics.ResScaleCustom.Value = CustomResolutionScale;
            }
            else if (ResolutionScale >= 0 && ResolutionScale < 7)
            {
                // Use custom scale mechanism for all presets (including lower resolutions)
                config.Graphics.ResScale.Value = -1;
                config.Graphics.ResScaleCustom.Value = GetResolutionScaleValue(ResolutionScale);
            }
            else
            {
                config.Graphics.ResScale.Value = 1; // Fallback to native
                config.Graphics.ResScaleCustom.Value = 1.0f;
            }
