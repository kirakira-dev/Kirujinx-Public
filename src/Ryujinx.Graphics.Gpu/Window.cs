...
namespace Ryujinx.Graphics.Gpu
{
    ...
    public class Window
    {
        ...
        private long _lastFrameTicks = System.Diagnostics.Stopwatch.GetTimestamp();
        ...
        public void Present(Action swapBuffersCallback)
        {
            ...
            double frameTimeMs = (currentTicks - _lastFrameTicks) * 1000.0 / System.Diagnostics.Stopwatch.Frequency;
            _lastFrameTicks = currentTicks;

            Ryujinx.Common.Utilities.PerformanceMode.RecordFrame(frameTimeMs);
            _context.DynamicResolution.RecordFrameTime(frameTimeMs);

            if (Ryujinx.Common.Utilities.PerformanceMode.Enabled && !_context.FrameSkip.ShouldRenderFrame())
            {
                if (_frameQueue.TryDequeue(out PresentationTexture skippedPt))
                {
                    skippedPt.ReleaseCallback(skippedPt.UserObj);
                }
                return;
            }

            Ryujinx.Common.Utilities.FpsLockManager.BeginFrame();
            _context.FramePacing.BeginFrame();
            _context.WorkScheduler.BeginFrame();
            _context.AdvanceSequence();

            if (!_context.SceneTransition.InGracePeriod)
            {
                _context.WorkScheduler.ProcessDeferredWork();
            }
            ...
            if (_frameQueue.TryDequeue(out PresentationTexture pt))
            {
                ...
                _context.SceneTransition.EndFrame();
                _context.FrameSkip.RecordFrameComplete();

                Ryujinx.Common.Utilities.FpsLockManager.EndFrameAndWait();
            }
        }
