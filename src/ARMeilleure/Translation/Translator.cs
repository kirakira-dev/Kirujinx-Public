...
namespace ARMeilleure.Translation
{
    public class Translator
    {
        ...
        internal SpeculativeTranslator SpeculativeTranslator { get; private set; }
        internal PriorityTranslationQueue PriorityQueue { get; }
        ...
        public Translator(IJitMemoryAllocator allocator, IMemoryManager memory, IAddressTable<ulong> functionTable)
        {
            ...
            PriorityQueue = new PriorityTranslationQueue();
            ...
            SpeculativeTranslator = new SpeculativeTranslator(this);
        }
        ...
        public void Execute(State.ExecutionContext context, ulong address)
        {
            if (Interlocked.Increment(ref _threadCount) == 1)
            {
                if (_ptc.State == PtcState.Enabled)
                {
                    ...
                    _ptc.PerformFullTranslationIfEnabled(this);
                }
                ...
                int scaledCount = (int)Math.Ceiling(baseCount * 1.5f);
                int unboundedThreadCount = scaledCount;
                int threadCount = Math.Clamp(unboundedThreadCount, 2, Math.Max(8, processorCount));
            ...
            if (Interlocked.Decrement(ref _threadCount) == 0)
            {
                ...
                SpeculativeTranslator?.Dispose();
                SpeculativeTranslator = null;
        ...
        internal TranslatedFunction GetOrTranslate(ulong address, ExecutionMode mode)
        {
            if (!Functions.TryGetValue(address, out TranslatedFunction func))
            {
                ...
                Ryujinx.Common.Utilities.StutterReductionManager.RecordTranslation(address, elapsedMs);
            }

            SpeculativeTranslator?.RecordExecution(address);
