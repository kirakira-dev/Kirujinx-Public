...
namespace ARMeilleure.Translation.PTC
{
    ...
    class Ptc : IPtcLoadState
    {
        ...
        public FullTranslationCache FullTranslation { get; }
        ...
        public Ptc()
        {
            ...
            FullTranslation = new FullTranslationCache();
        ...
        public void PerformFullTranslationIfEnabled(Translator translator)
        {
            if (!FullTranslationCache.EnableFullTranslation)
            {
                return;
            }

            ulong codeStart = Profiler.StaticCodeStart;
            ulong codeSize = Profiler.StaticCodeSize;

            if (codeSize == 0)
            {
                Logger.Warning?.Print(LogClass.Ptc, "Full translation skipped: No code region defined");
                return;
            }

            FullTranslation.StateChanged += (state, current, total) =>
            {
                PtcStateChanged?.Invoke(state switch
                {
                    FullPtcState.Scanning => PtcLoadingState.Start,
                    FullPtcState.Translating => PtcLoadingState.Loading,
                    FullPtcState.Completed => PtcLoadingState.Loaded,
                    _ => PtcLoadingState.Start
                }, current, total);
            };

            FullTranslation.PerformFullTranslation(translator, translator.Memory, codeStart, codeSize);
        }
        ...
        public void MakeAndSaveTranslations(Translator translator)
        {
            ...
            {
                baseParallelism = (int)Math.Ceiling(baseParallelism * 1.5f);
            }

            int degreeOfParallelism = baseParallelism;
            if (degreeOfParallelism > 4 && !Optimizations.LowPower)
            {
                degreeOfParallelism--;
            }

            degreeOfParallelism = Math.Clamp(degreeOfParallelism, 2, Math.Max(processorCount * 2, 16));
