...
namespace Ryujinx.Graphics.OpenGL.Image
{
    class TextureStorage : ITextureInfo
    {
        ...
        private void CreateImmutableStorage()
        {
            ...
            int maxSize = Math.Max(Info.Width, Math.Max(Info.Height, Info.Depth));
            int maxLevels = 1 + BitOperations.Log2((uint)Math.Max(1, maxSize));
            int levels = Math.Min(Info.Levels, maxLevels);
